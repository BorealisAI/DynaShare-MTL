...
class CityScapes(torch.utils.data.Dataset):
    ...
    def __scale__(img, depth, label2, label7, label19):
        ...
        depth = np.expand_dims(
            cv2.resize(depth, (w_new, h_new), interpolation=cv2.INTER_NEAREST), axis=-1
        )
        label2 = np.expand_dims(
            cv2.resize(label2, (w_new, h_new), interpolation=cv2.INTER_NEAREST), axis=-1
        )
        label7 = np.expand_dims(
            cv2.resize(label7, (w_new, h_new), interpolation=cv2.INTER_NEAREST), axis=-1
        )
        label19 = np.expand_dims(
            cv2.resize(label19, (w_new, h_new), interpolation=cv2.INTER_NEAREST),
            axis=-1,
        )
    ...
    def __random_crop_and_pad_image_and_labels__(
        img, depth, label2, label7, label19, crop_h, crop_w, ignore_label=-1.0
    ):
        # combining
        ...
        depth_crop = deepcopy(
            combined_crop[:, :, img_cdim : img_cdim + depth_cdim]
        ).astype("float")
    ...
    def __getitem__(self, item):
        ...
        label2 = np.expand_dims(
            np.load(os.path.join(self.dataroot, label2_path)), axis=-1
        )
        label7 = np.expand_dims(
            np.load(os.path.join(self.dataroot, label7_path)), axis=-1
        )
        label19 = np.expand_dims(
            np.load(os.path.join(self.dataroot, label19_path)), axis=-1
        )
        if self.mode in ["train", "train1", "train2"]:
            img, depth, label2, label7, label19 = self.__scale__(
                img, depth, label2, label7, label19
            )
            img, depth, label2, label7, label19 = self.__mirror__(
                img, depth, label2, label7, label19
            )
            (
                img,
                ...
                img, depth, label2, label7, label19, self.crop_h, self.crop_w
            )
        ...
        batch = {
            ...
            "label2": torch.from_numpy(label2).permute(2, 0, 1),
            "label7": torch.from_numpy(label7).permute(2, 0, 1),
            "label19": torch.from_numpy(label19).permute(2, 0, 1),
            "seg": seg,
            "name": name,
        }
        ...
        if self.opt is not None:
            policy_dir = os.path.join(
                self.opt["paths"]["result_dir"], self.opt["exp_name"], "policy"
            )
