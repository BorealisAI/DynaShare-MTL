...
class NYU_v2(torch.utils.data.Dataset):
    ...
    def __scale__(img, label1, label2, label3):
        ...
        label1 = np.expand_dims(
            cv2.resize(label1, (w_new, h_new), interpolation=cv2.INTER_NEAREST), axis=-1
        )
        label2 = cv2.resize(label2, (w_new, h_new), interpolation=cv2.INTER_NEAREST)
        label3 = np.expand_dims(
            cv2.resize(label3, (w_new, h_new), interpolation=cv2.INTER_NEAREST), axis=-1
        )
    ...
    def __random_crop_and_pad_image_and_labels__(
        img, label1, label2, label3, crop_h, crop_w, ignore_label=255
    ):
        # combining
    ...
    def __getitem__(self, item):
        ...
        seg = np.expand_dims(
            cv2.imread(os.path.join(self.dataroot, seg_path), cv2.IMREAD_GRAYSCALE),
            axis=-1,
        )
        normal = cv2.imread(os.path.join(self.dataroot, normal_path))
        depth = np.expand_dims(
            np.load(os.path.join(self.dataroot, depth_path)), axis=-1
        )
        if self.mode in ["train", "train1", "train2"]:
            ...
            img, seg, normal, depth = self.__random_crop_and_pad_image_and_labels__(
                img, seg, normal, depth, self.crop_h, self.crop_w
            )
        ...
        if not os.path.exists(os.path.join(dataroot, "nyu_v2_3task.json")):
            ...
            for f_name in os.listdir(os.path.join(dataroot, img_dir)):
                if f_name.endswith("png"):
                    ...
                    if (
                        os.path.exists(os.path.join(dataroot, img_path))
                        and os.path.exists(os.path.join(dataroot, seg_path))
                        and os.path.exists(os.path.join(dataroot, sn_path))
                        and os.path.exists(os.path.join(dataroot, depth_path))
                    ):
                        json_file["test"].append(
                            (img_path, seg_path, sn_path, depth_path)
                        )
...
class NYU_v2_Single_IMG(NYU_v2):
    def __init__(
        self, dataroot, mode, img_id, crop_h=None, crop_w=None, policy_dir=None
    ):
        super(NYU_v2_Single_IMG, self).__init__(
            dataroot, mode, crop_h, crop_w, policy_dir
        )
    ...
    def __getitem__(self, item):
        ...
        if self.policy_dir is not None:
            ...
            return {
                ...
                "depth": torch.from_numpy(depth).permute(2, 0, 1),
                "name": name,
                "policy1": torch.from_numpy(policy1),
                "policy2": torch.from_numpy(policy2),
            }
