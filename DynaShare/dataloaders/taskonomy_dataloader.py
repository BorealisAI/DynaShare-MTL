...
class Taskonomy(torch.utils.data.Dataset):
    def __init__(self, dataroot, mode, crop_h=None, crop_w=None):
        ...
        self.IMG_MEAN = np.tile(
            self.IMG_MEAN[np.newaxis, np.newaxis, :], (self.crop_h, self.crop_w, 1)
        )
    ...
    def __scale__(
        ...
        seg_p = np.expand_dims(
            cv2.resize(seg_p, (w_new, h_new), interpolation=cv2.INTER_NEAREST), axis=-1
        )
        seg_mask = np.expand_dims(
            cv2.resize(seg_mask, (w_new, h_new), interpolation=cv2.INTER_NEAREST),
            axis=-1,
        )
        sn_p = cv2.resize(sn_p, (w_new, h_new), interpolation=cv2.INTER_NEAREST)
        depth_p = np.expand_dims(
            cv2.resize(depth_p, (w_new, h_new), interpolation=cv2.INTER_NEAREST),
            axis=-1,
        )
        depth_mask = np.expand_dims(
            cv2.resize(depth_mask, (w_new, h_new), interpolation=cv2.INTER_NEAREST),
            axis=-1,
        )
        keypoint_p = np.expand_dims(
            cv2.resize(keypoint_p, (w_new, h_new), interpolation=cv2.INTER_NEAREST),
            axis=-1,
        )
        edge_p = np.expand_dims(
            cv2.resize(edge_p, (w_new, h_new), interpolation=cv2.INTER_NEAREST), axis=-1
        )
    ...
    @staticmethod
    def __random_crop_and_pad_image_and_labels__(
        img_p,
        seg_p,
        seg_mask,
        sn_p,
        ...
        label = np.concatenate(
            (seg_p, seg_mask, sn_p, depth_p, depth_mask, keypoint_p, edge_p), axis=2
        ).astype("float32")
        label -= ignore_label
        ...
        pad_shape = [
            ...
            max(image_shape[1], crop_w),
            combined.shape[-1],
        ]
        combined_pad = np.zeros(pad_shape)
        offset_h, offset_w = (
            (pad_shape[0] - image_shape[0]) // 2,
            (pad_shape[1] - image_shape[1]) // 2,
        )
        combined_pad[
            offset_h : offset_h + image_shape[0], offset_w : offset_w + image_shape[1]
        ] = combined
        # cropping
        crop_offset_h, crop_offset_w = pad_shape[0] - crop_h, pad_shape[1] - crop_w
        start_h, start_w = (
            np.random.randint(0, crop_offset_h + 1),
            np.random.randint(0, crop_offset_w + 1),
        )
        ...
        depth_mask_crop = deepcopy(
            combined_crop[:, :, sum(c_dims[:5]) : sum(c_dims[:6])]
        )
    ...
    @staticmethod
    def rescale_image(img, new_scale=(-1.0, 1.0), current_scale=None, no_clip=False):
        """
    ...
    def resize_rescale_image(
        ...
        img = img.astype("float32")
        img = cv2.resize(img, new_dims, interpolation=cv2.INTER_NEAREST)
        img = self.rescale_image(
            img, new_scale, current_scale=current_scale, no_clip=no_clip
        )
        return img

    def resize_and_rescale_image_log(
        self, img, new_dims=(256, 256), offset=1.0, normalizer=np.log(2.0 ** 16)
    ):
        """
    ...
    @staticmethod
    def mask_if_channel_ge(
        img, threshold, channel_idx, broadcast_to_shape=None, broadcast_to_dim=None
    ):
        """
    ...
    def make_depth_mask(self, img, new_dims=(256, 256), broadcast_to_dim=1):
        target_mask = self.mask_if_channel_ge(
            img, threshold=64500, channel_idx=0, broadcast_to_dim=broadcast_to_dim
        )
        target_mask = cv2.resize(target_mask, new_dims, interpolation=cv2.INTER_NEAREST)
        target_mask[target_mask < 0.99] = 0.0
        return target_mask
    ...
    def __getitem__(self, item):
        while True:
            ...
            try:
                img = np.array(
                    Image.open(os.path.join(self.dataroot, img_path))
                ).astype("float32")[:, :, ::-1]
                ...
                sn = (
                    np.array(Image.open(os.path.join(self.dataroot, sn_path))).astype(
                        "float32"
                    )
                    / 255
                )
                ...
                depth = np.array(
                    Image.open(os.path.join(self.dataroot, depth_path))
                ).astype("float32")
                depth_p = self.resize_and_rescale_image_log(depth)
                depth_mask = self.make_depth_mask(depth)
                keypoint = np.array(
                    Image.open(os.path.join(self.dataroot, keypoint_path))
                ).astype("float32") / (2 ** 16)
                keypoint_p = self.resize_rescale_image(
                    keypoint, current_scale=(0, 0.005)
                )
                edge = np.array(
                    Image.open(os.path.join(self.dataroot, edge_path))
                ).astype("float32") / (2 ** 16)
        ...
        if self.mode in ["train", "train1", "train2"]:
            ...
            (
                ...
                keypoint_p,
                edge_p,
            ) = self.__random_crop_and_pad_image_and_labels__(
                img_p,
                seg_p,
                seg_mask,
                sn_p,
        ...
        return {
            "img": torch.from_numpy(img_p).permute(2, 0, 1).float(),
