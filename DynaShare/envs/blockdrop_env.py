...
class BlockDropEnv(BaseEnv):
    ...
    def define_networks(self, tasks_num_class, coef):
        ...
        if self.opt["backbone"].startswith("ResNet"):
            ...
            if self.opt["policy_model"] == "task-specific":
                ...
                self.networks["mtl-net"] = MTL_Instance(
                    block,
                    layers,
                    tasks_num_class,
                    init_method,
    ...
    def optimize_fix_policy(self, lambdas, target_rates, num_train_layer=None):
        self.forward_fix_policy(num_train_layer, target_rates)
        if "seg" in self.tasks:
            seg_num_class = self.tasks_num_class[self.tasks.index("seg")]
            self.get_seg_loss(seg_num_class)
        if "sn" in self.tasks:
    ...
    def val_fix_policy(self, target_rates, flag_act=False, num_train_layers=None):
        metrics = {}
        self.forward_fix_policy(num_train_layers, target_rates, flag_act)
        self.resize_results()
        if "seg" in self.tasks:
            metrics["seg"] = {}
            seg_num_class = self.tasks_num_class[self.tasks.index("seg")]
    ...
    def forward_fix_policy(self, num_train_layers, target_rates, flag_act=False):
        ...
        if (
            flag_act
        ):  # There is a need to compute the actual activation-rates during the test phase
            self.act1.append(torch.tensor(self.gate_activation1))
            self.act2.append(torch.tensor(self.gate_activation2))
        ...
        if self.opt["policy_model"] == "task-specific":
            for t_id in range(self.num_tasks):
                if isinstance(self.networks["mtl-net"], nn.DataParallel):
                    ...
                    logits = getattr(
                        self.networks["mtl-net"], "task%d_logits" % (t_id + 1)
                    )
            ...
            for t_id in range(self.num_tasks):
                ...
                if self.opt["diff_sparsity_weights"] and not self.opt["is_sharing"]:
                    ...
                    loss_weights = (
                        loss_weights.view(1, -1).repeat(batch_size, 1).view(-1)
                    )

                    self.losses["sparsity"]["task%d" % (t_id + 1)] = (
                        2
                        * (
                            loss_weights[-batch_size * num_blocks :]
                            * self.cross_entropy2(logit, gt)
                        ).mean()
                    )
                else:
                    self.losses["sparsity"][
                        "task%d" % (t_id + 1)
                    ] = self.cross_entropy_sparsity(logit, gt)
    ...
    def backward_policy(self, lambdas, num_train_layers):
        ...
        loss += self.opt["train"]["targetrate_w"] * self.losses["targetrate"]["total"]
    ...
    def get_policy_prob(self):
        ...
        if self.opt["policy_model"] == "task-specific":
            ...
            raise ValueError(
                "policy mode = %s is not supported" % self.opt["policy_model"]
            )
    ...
    def load_snapshot(self, snapshot, temperature):
