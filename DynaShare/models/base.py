...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, dilation=1):
        ...
        # Gate layers
        self.fc1 = nn.Conv2d(inplanes, 16, kernel_size=1)
        self.fc1bn = nn.BatchNorm2d(16)
        self.fc2 = nn.Conv2d(16, 2, kernel_size=1)
        self.gs = GumbelSoftmax()
        self.gs.cuda()

    def forward(self, x, temperature=1, keep=None):
        # keep: [batch_size], int
        ...
        w = F.gumbel_softmax(w, temperature, hard=True, dim=1)
        out = self.conv1(x)
        out = self.bn1(out)
...
class Classification_Module(nn.Module):
    def __init__(self, inplanes, num_classes, coef, rate=12):
