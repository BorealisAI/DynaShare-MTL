...
def eval_fix_policy(
    target_rates, environ, dataloader, tasks, num_seg_cls=-1, eval_iter=10
):
    batch_size = []
    records = {}
    val_metrics = {}
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader)):
            ...
            metrics = environ.val_fix_policy(target_rates)
            if "seg" in tasks:
...
def _train(exp_id, opt, gpu_ids):
    ...
    if opt["train"]["retrain_resume"]:
        ...
        if opt["policy_model"] == "task-specific":
            ...
            if opt["retrain_from_pl"]:
                environ.load(opt["train"]["policy_iter"], opt["train"]["init_temp"])
            else:
    ...
    while current_iter < opt["train"]["retrain_total_iters"]:
        ...
        environ.set_inputs(batch)
        environ.optimize_fix_policy(opt["lambdas"], opt["target_rates"])

        if should(current_iter, opt["train"]["print_freq"]):
            environ.print_loss(current_iter, start_time, exp_id)
        ...
        if should(current_iter, opt["train"]["val_freq"]):
            ...
            val_metrics = eval_fix_policy(
                opt["target_rates"], environ, val_loader, opt["tasks"], num_seg_class
            )
            environ.print_loss(current_iter, start_time, exp_id, val_metrics)
            environ.save(
...
def train():
    ...
    with open(
        os.path.join(
            opt["paths"]["log_dir"], opt["exp_name"], opt["exp_subname"], "opt.txt"
        ),
        "w+",
    ) as f:
        f.writelines(lines)
