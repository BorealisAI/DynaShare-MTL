...
import torch
from utils.util import print_separator, read_yaml, create_path, print_yaml,fix_random_seed
from sklearn.metrics import confusion_matrix
from tqdm import tqdm


def eval_fix_policy(target_rates,environ, dataloader, tasks, num_seg_cls=-1, eval_iter=10):
    batch_size = []
    records = {}
    val_metrics = {}
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader)):
            ...
            metrics = environ.val_fix_policy(target_rates, flag_act = True)
            if "seg" in tasks:
...
def test():
    ...
    val_loader = DataLoader(
        valset, batch_size=1, drop_last=True, num_workers=2, shuffle=False
    )
    ...
    environ.eval()
    val_metrics = eval_fix_policy(opt['target_rates'],
        environ, val_loader, opt["tasks"], num_seg_cls=num_seg_class, eval_iter=-1
    )
    print(val_metrics)
    activation_rate_task1 = torch.stack(environ.act1).mean(axis=0)
    activation_rate_task2 = torch.stack(environ.act2).mean(axis=0)
    # Consider only the activation_rate for the last 4 layers
    print(activation_rate_task1, activation_rate_task2)
