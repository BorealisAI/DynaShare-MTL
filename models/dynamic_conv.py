...
class Dynamic_conv1d(nn.Module):
    def __init__(
        ...
        self.weight = nn.Parameter(
            torch.randn(K, out_planes, in_planes // groups, kernel_size),
            requires_grad=True,
        )
        if bias:
    ...
    def forward(self, x):
        ...
        aggregate_weight = torch.mm(softmax_attention, weight).view(
            -1, self.in_planes, self.kernel_size,
        )
...
class Dynamic_conv3d(nn.Module):
    ...
    def forward(self, x):
        ...
        aggregate_weight = torch.mm(softmax_attention, weight).view(
            -1, self.in_planes, self.kernel_size, self.kernel_size, self.kernel_size
        )
        if self.bias is not None:
            aggregate_bias = torch.mm(softmax_attention, self.bias).view(-1)
            output = F.conv3d(
...
if __name__ == "__main__":
    ...
    model = Dynamic_conv1d(
        in_planes=3, out_planes=16, kernel_size=3, ratio=0.25, padding=1,
    )
    x = x.to("cuda:0")
    model.to("cuda")
    print(model(x).shape)
